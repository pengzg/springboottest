tempApp.controller('ctr_productRedAdd', function($scope, http, messageFactory, $state, $stateParams, EzConfirm) {
	...
	if ($scope.pm_id) {
		...
		$scope.vo = {"pm_state":1,"pm_integral_mall":'N',"pm_typeid":'1002',"pm_market_price":1,"pm_payment_type":1,"pm_integral_value":0.1,"pm_stock":0};
	}
    ...
    $scope.saveProduct = function(){
    	...
    	if (!$scope.vo.pm_subtitle) {
    		messageFactory.showMessage('error',"请输入祝福语");
    		return false;
    	}
    	
    	if (!$scope.vo.pm_price) {
    		messageFactory.showMessage('error',"请输入红包金额");
    		return false;
    	}
