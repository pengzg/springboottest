tempApp.controller('ctr_rechargeAdd', function($scope, http, messageFactory, $state, $stateParams, EzConfirm,dateUtil) {
    $scope.pager = { page: 1, rows: "10", sort: "pm_id", order: "desc", pageList: ["10", "20", "30"] };
    $scope.unitVO = {};
    $scope.skuList = [{ps_code:"P"+dateUtil.getTs()}];
    $scope.vo = {pm_rebate_ratio:"0.00",pm_delivery_type:"",pm_delivery_fee_type:1,pm_market_price:0,pm_price:0,pm_typeid:$stateParams.pt_id,'pm_state':1,"pm_isgroup":"N","pm_valid_day":1,"pm_group_price":1,"pm_group_peopler_num":2, "pm_def3":"0.00"};
    $scope.picList = [];
    $scope.priceList = [{"psp_rebate_type":1, "psp_rebate_value":"0"}];
     ...
     */
		...
		for(i in $scope.priceList){
            ...
            if ($scope.priceList[i].psp_price<=0) {
                messageFactory.showMessage('error',"充值金额必须大于0元");
                return false;
            }
			var priceInfo = {}
            var item = $scope.priceList[i];
            if (!($scope.priceList[i].psp_rebate_value+"")) {
                messageFactory.showMessage('error',"请输入返现值");
				return false;
            }
            if ($scope.priceList[i].psp_rebate_value<0) {
                messageFactory.showMessage('error',"返现值必须大于等于0");
                return false;
            }
			priceInfo = item;
        ...
        var url = "/admin/product/productMainControl/addRecharge.action";
