tempApp.controller('ctr_csLineUserList', function($scope,
		$state,http,EzConfirm,messageFactory) {
	$scope.pager = {page:1,rows:'10',sort:'clu_id',order:'desc',pageList:['10','20','30']};
	$scope.searchParam = {};
	
	/**
	 * 查询数据
	 */
	var queryCsLineUser = function(){
		messageFactory.showLoading();
		var success = function(result){
			$scope.csLineUserList = result.data.rows;
		...
		var url = '/admin/line/csLineUserControl/dataGrid.action';
	...
	$scope.toEdit = function(id){
		$state.go('index.csLineUserAdd',{clu_id:id})
	}
	...
	$scope.toDetail = function(id){
		$state.go('index.csLineUserDetail',{clu_id:id})
	}
	...
	$scope.toDelete = function(id){
		...
		var data ={'clu_id':id}
		var url = "/admin/line/csLineUserControl/delete.action";
		var	msg = '您确定修改本条记录吗？';
		EzConfirm.create({
			heading : '提示',
			text : msg
